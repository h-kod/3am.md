...
2. Copy the logs out before rotation: `sudo journalctl --since '-2h' > ~/incident-LOGS.txt` `[linux]` · `kubectl logs POD --since=2h --previous > ~/incident-LOGS.txt` `[k8s]`
   → `previous terminated container not found` → nothing restarted in place; drop the flag and re-run · the whole pod was replaced → `kubectl logs` cannot reach it either way, so go to your log backend now, not later · an empty file `[linux]` → `sudo` matters, and a journal with `Storage=volatile` is destroyed by the reboot you are investigating; try `journalctl -b -1`
...
6. `crontab -l` and `kubectl get cronjobs -A` `[k8s]`, then compare their schedules with your failure timestamp — CronJob schedules run in UTC unless `.spec.timeZone` is set.
...
*Verified: macOS 26.5, system tools · 2026-08. `crontab -l`, `ps -o lstart`, `uptime` run as written. The `journalctl` and `kubectl` paths are NOT runtime-verified here — see the [run matrix](../README.md#where-each-file-has-been-run). Step 2's branches are the ones to confirm first: both the `--previous` error and the volatile-journal case produce a plausible file that is missing the incident.*
