...
2. `du -xh --max-depth=1 / 2>/dev/null | sort -h | tail -10` `[linux]` · `du -xhd1 / 2>/dev/null | sort -h | tail -10` `[macos]`
   → one directory dominates → descend into it by repeating the command · nothing dominates → the space is held by deleted-but-open files, go to 3
