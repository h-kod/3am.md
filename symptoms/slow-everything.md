...
4. `psql -h DB_HOST -c "select pid, now()-query_start as age, state, left(query,60) from pg_stat_activity where state <> 'idle' order by age desc limit 5;"` · `[mysql]` `SHOW FULL PROCESSLIST;`
   → ages in seconds → `db-connections-exhausted.md` · nothing running long → 5
...
6. `vmstat 1 5` `[linux]` · `vm_stat 1 5` and `iostat 1 5` `[macos]`
   → swap in/out non-zero → `oom-killed.md` · high IO wait → `disk-full.md` · CPU pinned with nothing queued → you are simply out of capacity

**Nothing matched?** Open an issue with the exact `curl -w` line you got.

*Verified: macOS 26.5, system tools · 2026-08. Steps 1-3, 5 and the macOS half of 6 run as written. Step 4 and `vmstat` are not runtime-verified here.*
