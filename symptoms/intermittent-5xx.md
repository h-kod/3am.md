...
4. `kubectl get endpoints SERVICE -o wide` `[k8s]` · `aws elbv2 describe-target-health --target-group-arn ARN` `[aws]`
   → fewer ready than expected → the LB is doing its job; find why that target died → `oom-killed.md` or `disk-full.md` · all healthy → 5
...
*Verified: macOS 26.5, system tools · 2026-08. Steps 1-3, 5 and 6 run as written. Step 4 needs a live cluster — not runtime-verified here.*
